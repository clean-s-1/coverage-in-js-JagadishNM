...
describe("classifyTemperatureBreach", () => {
  it("Classify temperature breach with PASSIVE_COOLING type to return TOO_LOW", () => {
    expect(alerts.classifyTemperatureBreach('PASSIVE_COOLING', -20))
    .equals("TOO_LOW");
  });

  it("Classify temperature breach with PASSIVE_COOLING type to return TOO_HIGH", () => {
    expect(alerts.classifyTemperatureBreach('PASSIVE_COOLING', 50))
    .equals("TOO_HIGH");
  });

  it("Classify temperature breach with PASSIVE_COOLING type to return NORMAL", () => {
    expect(alerts.classifyTemperatureBreach('PASSIVE_COOLING', 20))
    .equals("NORMAL");
  });

  it("Classify temperature breach with HI_ACTIVE_COOLING type to return TOO_LOW", () => {
    expect(alerts.classifyTemperatureBreach('HI_ACTIVE_COOLING', -2))
    .equals("TOO_LOW");
  });

  it("Classify temperature breach with HI_ACTIVE_COOLING type to return TOO_HIGH", () => {
    expect(alerts.classifyTemperatureBreach('HI_ACTIVE_COOLING', 48))
    .equals("TOO_HIGH");
  });

  it("Classify temperature breach with HI_ACTIVE_COOLING type to return NORMAL", () => {
    expect(alerts.classifyTemperatureBreach('HI_ACTIVE_COOLING', 30))
    .equals("NORMAL");
  });

  it("Classify temperature breach with MED_ACTIVE_COOLING type to return TOO_LOW", () => {
    expect(alerts.classifyTemperatureBreach('MED_ACTIVE_COOLING', -5))
    .equals("TOO_LOW");
  });

  it("Classify temperature breach with MED_ACTIVE_COOLING type to return TOO_HIGH", () => {
    expect(alerts.classifyTemperatureBreach('MED_ACTIVE_COOLING', 45))
    .equals("TOO_HIGH");
  });

  it("Classify temperature breach with MED_ACTIVE_COOLING type to return NORMAL", () => {
